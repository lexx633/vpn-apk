...
object SettingsManager {
    ...
    /**
     * limm: по умолчанию российские сайты идут мимо VPN (напрямую) — пресет WHITE_RUSSIA:
     * geosite:category-ru + geoip:ru → outbound "direct". Чтобы geoip:ru срабатывал и при
     * обращении по домену (а не только по IP), domainStrategy = IPIfNonMatch.
     * Одноразовая миграция: применяется и к свежим установкам, и к уже стоящим (где был WHITE).
     */
    private fun migrateRoutingToRussia(context: Context) {
        val key = "limm_ru_routing_v1"
