...
class SubEditActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (subItem != null) {
            ...
            clearServer()
            // limm: allow pre-filling URL/remarks when launched from the "+" menu shortcut
            intent.getStringExtra("prefillUrl")?.takeIf { it.isNotEmpty() }?.let {
                binding.etUrl.text = Utils.getEditable(it)
            }
            intent.getStringExtra("prefillRemarks")?.takeIf { it.isNotEmpty() }?.let {
                binding.etRemarks.text = Utils.getEditable(it)
            }
        }
    }
